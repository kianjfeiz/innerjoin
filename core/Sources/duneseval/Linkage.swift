...
enum Linkage {
    ...
    static func run(pairsAt path: String) throws {
        ...
        results.append(("dunes — any match kind", score(pairs) {
            Resolver.matchKind($0, $1) != nil
        }))
        results.append(("dunes — decides alone (rung 4)", score(pairs) {
        ...
        for threshold in [0.88, 0.90, 0.92, 0.94] {
            results.append(("dunes + similarity ≥ \(threshold)", score(pairs) {
        ...
        for ceiling in [1, 2, 3] {
            results.append(("dunes + rarity (surname ≤ \(ceiling))", score(pairs) { left, right in
