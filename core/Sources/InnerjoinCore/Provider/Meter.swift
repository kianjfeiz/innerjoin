...
public actor Meter {
    ...
    public struct Pricing: Sendable {
        ...
        /// From `IJ_PRICE_IN` / `IJ_PRICE_OUT`. Absent means no dollar figure is shown —
        /// prices change, and a stale hardcoded table would quietly lie.
        public static func fromEnvironment(
            _ environment: [String: String] = ProcessInfo.processInfo.environment
        ) -> Pricing? {
            guard let input = environment["IJ_PRICE_IN"].flatMap(Double.init),
                  let output = environment["IJ_PRICE_OUT"].flatMap(Double.init)
            else { return nil }
